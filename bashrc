...
#$host_color="$(random_color $HOSTNAME)"
  export PS1='\[${RESET}${RED}\][\[${BASE01}\]\t\[${RED}\]] [\[${YELLOW}\]\u\[${CYAN}\]@\[${MAGENTA}\]\h\[${RED}\]] \[${BASE00}\]($(for r in ${PIPESTATUS[*]} ; do [ $r -eq 0 ] && echo -n "\[$BASE01\] $r" || echo -n " \[${RED}\]${r}\[${RESET}\]" ; done)\[${BASE00}\] ) \[${CYAN}\]|:\w$(is_git_repo && echo -n " \[${GREEN}\]$(git_branch)\[${RESET}\]" && is_git_dirty && echo -n "\[${RED}\]*\[${RESET}\]")\n\[${BASE00}\]-> \[$RED\]\$ \[$RESET\]'
if [[ `uname` == "Darwin" ]]; then
