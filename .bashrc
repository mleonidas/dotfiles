...
#$host_color="$(random_color $HOSTNAME)"
  export PS1='\[${RESET}${BASE02}\][\[${BASE0BASE01}\]\t\[${BASE02}\]] [\[${YELLOW}\]\u\[${CYAN}\]@\[${MAGENTA}\]\h\[${BASE02}\]] \[${BASE00}\]($(for r in ${PIPESTATUS[*]} ; do [ $r -eq 0 ] && echo -n "\[$BASE01\] $r" || echo -n " \[${RED}\]${r}\[${RESET}\]" ; done)\[${BASE00}\] ) \[${ORANGE}\]\w$(is_git_repo && echo -n " \[${GREEN}\]$(git_branch)\[${RESET}\]" && is_git_dirty && echo -n "\[${RED}\]*\[${RESET}\]")\n\[${BASE00}\]-> \[$RED\]\$ \[$RESET\]'
if [[ `uname` == "Darwin" ]]; then
